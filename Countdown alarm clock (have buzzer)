...
sbit k2=P1^1;	 //分+	键断开 P1^1=1<>k2=1	;  键闭合 P1^1=0<>k2=0	;
sbit beep=P1^4;
...
int shi=23,fen=59,miao=48;	 //改这里可以决定开始的时间,它在上一次的基础上加入了蜂鸣器
int count=0;
...
void naoling()
{
  if((shi==0)&&(fen==0)&&(miao==0))//00-00-00
  {
  	while((miao<6)&&(TR0==1))//响铃，响5s
	{
	  key();
	  fenli();
	  display(); 
